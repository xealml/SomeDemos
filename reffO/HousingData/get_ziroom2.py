...
def get_and_save():
    def get_in_page_information(page):
        ...
        for i in range(2, 19):
            id = \
            beautifulsoup.select('#houseList > li:nth-of-type(' + str(i) + ')> div.txt > h3 > a')[0].get('href').split(
                '/')[-1].split('.')[0]
            ...
            information = \
            beautifulsoup.select('#houseList > li:nth-of-type(' + str(i) + ')> div.txt > div > p:nth-of-type(1)')[
                0].text.replace('\n', '').replace(' ', '').split('|')
            ...
            distance_from_subway = beautifulsoup.select(
                '#houseList > li:nth-of-type(' + str(i) + ') > div.txt > div > p:nth-of-type(2) > span')[0].text
            # 价格 #houseList > li:nth-child(2) > div.priceDetail > p.price
            price = beautifulsoup.select('#houseList > li:nth-of-type(' + str(i) + ') > div.priceDetail > p.price')[
                0].text.replace('\n', '').replace(' ', '')
